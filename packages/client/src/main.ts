...
/**
 * The survivor model, loaded ONCE and cloned per body.
 *
 * Generated by tools/blender/build_survivor.py rather than downloaded, so the
 * character pipeline stays script-reproducible and free of an Adobe account.
 * 406 verts, 19 bones, 51 KB — sized for the GTX 1650, not for a hero shot.
 */
let survivorProto: THREE.Group | null = null;
try {
  const gltf = await new GLTFLoader().loadAsync("/models/survivor.glb");
  survivorProto = gltf.scene;
} catch {
  console.warn("[ashfall] survivor.glb missing; falling back to capsules");
}
...
/** True when there is no rigged head to mount the lamp on. */
let headlampFallback = true;
...
interface Avatar {
  group: THREE.Group;
  /** Null when survivor.glb failed to load and we fell back to a capsule. */
  human: Humanoid | null;
...
function avatarFor(id: string, label: string, isSelf: boolean): Avatar {
  ...
  let human: Humanoid | null = null;
  let body: THREE.Mesh | null = null;

  if (survivorProto) {
    human = new Humanoid(survivorProto);
    // Tint per-instance so the crowd is not sixty identical suits. Cloning the
    // material is required — sharing it would recolour everyone at once.
    human.root.traverse((o) => {
      const m = o as THREE.Mesh;
      if (m.isMesh && m.material) {
        const mat = (m.material as THREE.MeshStandardMaterial).clone();
        mat.color.offsetHSL(0, 0, isSelf ? 0.06 : (Math.random() - 0.5) * 0.09);
        m.material = mat;
      }
    });
    ...
    body = new THREE.Mesh(
      avatarGeo,
      new THREE.MeshStandardMaterial({ color: isSelf ? 0xc8b89a : 0x7d8894, roughness: 0.75 })
    );
    body.position.y = 0.9;
  ...
  scene.add(group);
  const a: Avatar = { group, human, body, tag, label, prev: new THREE.Vector2(), bob: 0, facing: 0 };
  avatars.set(id, a);
  return a;
}
...
function frame(now: number): void {
  ...
  // Position is set AFTER the humanoid poses, further down, so the lamp
  // inherits breathing and gait sway from the head bone. This fallback only
  // runs when survivor.glb is missing.
  headlampFallback = true;
  ...
  const selfPos = new THREE.Vector3(net.self.x, 0, net.self.z);
  let selfHuman: Humanoid | null = null;
  if (net.selfId) {
    const me = avatarFor(net.selfId, name, true);
    if (me.human) {
      // Face the direction of travel; hold the last heading while standing
      // still, so stopping does not snap you back to north.
      if (moving) me.facing = Math.atan2(facing.x, facing.y);
      me.human.update(selfPos, me.facing, dt);
      me.group.position.set(0, 0, 0); // the Humanoid owns its own transform
      me.tag.position.set(net.self.x, 2.05, net.self.z);
      selfHuman = me.human;
      // THE HEADLAMP IS MOUNTED ON THE HEAD BONE, not the camera.
      //
      // Camera-mounted is how most web games do it and it is why their lights
      // feel like a flashlight taped to the monitor. On the bone, the beam
      // inherits the breathing cycle and the gait sway for free — the light
      // drifts a couple of centimetres while you stand still, which is most of
      // what sells "strapped to a person" rather than "floating in the lens".
      me.human.aimHeadlamp(headlamp, headlamp.target);
      headlampFallback = false;
    } else if (me.body) {
      me.group.position.copy(selfPos);
  ...
  for (const p of net.interpolated()) {
    ...
    const dx = p.x - a.prev.x;
    const dz = p.z - a.prev.y;
    const speed = Math.hypot(dx, dz) / Math.max(dt, 0.001);

    if (a.human) {
      // Remote players animate from their OWN observed motion, so a walk cycle
      // needs no extra protocol field — the position stream already carries it.
      if (speed > 0.25) a.facing = Math.atan2(dx, dz);
      a.human.update(new THREE.Vector3(p.x, 0, p.z), a.facing, dt);
      a.group.position.set(0, 0, 0);
      a.tag.position.set(p.x, 2.05, p.z);
    } else if (a.body) {
  ...
  for (const n of liveNpcs) {
    ...
    if (!npc) {
      ...
      if (survivorProto) {
        human = new Humanoid(survivorProto);
        // Occupation tint, per instance. The material must be cloned or the
        // whole crowd recolours together.
        const tint = OCCUPATION_TINT[n.occupation] ?? OCCUPATION_TINT.idle;
        human.root.traverse((o) => {
          const m = o as THREE.Mesh;
          if (m.isMesh && m.material) {
            const mat = (m.material as THREE.MeshStandardMaterial).clone();
            mat.color.lerp(new THREE.Color(tint), 0.45);
            mat.color.offsetHSL(0, 0, (Math.random() - 0.5) * 0.07);
            m.material = mat;
          }
        });
        scene.add(human.root);
      } else {
        mesh = new THREE.Mesh(npcGeo, npcMats.get(n.occupation) ?? npcMats.get("idle")!);
        mesh.castShadow = true;
        mesh.position.y = 0.86;
        scene.add(mesh);
      }
      npc = { mesh, human, bob: Math.random() * 6, prev: new THREE.Vector2(n.x, n.z), facing: 0 };
      npcs.set(n.id, npc);
    }

    const ndx = n.x - npc.prev.x;
    const ndz = n.z - npc.prev.y;
    const speed = Math.hypot(ndx, ndz) / Math.max(dt, 0.001);

    if (npc.human) {
      if (speed > 0.12) npc.facing = Math.atan2(ndx, ndz);
      npc.human.update(new THREE.Vector3(n.x, 0, n.z), npc.facing, dt);
    } else if (npc.mesh) {
  ...
  for (const [id, npc] of npcs) {
    ...
    if (npc.human) scene.remove(npc.human.root);
  ...
  if (headlampFallback) {
    ...
    headlamp.target.position.set(net.self.x + facing.x * 10, 0.9, net.self.z + facing.y * 10);
